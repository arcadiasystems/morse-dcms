...
import { buildReadContext, type ReadContext } from "../cli/context.ts";
...
export async function runUse(
	...
	makeContext: () => Promise<ReadContext>,
): Promise<void> {
	if (options.clear) {
...
/** Render the active context. Caller resolves `settings` (loadConfig + resolveSettings). */
export function runStatus(output: Output, settings: ResolvedSettings): void {
...
export function registerContextCommands(program: Command): void {
	...
	program
		...
		.action(async (_options, command: Command) => {
			runStatus(
				outputFor(command),
				resolveSettings(globalOptions(command), await loadConfig()),
			);
		});
}
