...
type DecryptOptions = TargetOptions & {
	out?: string;
	publisherCap?: string;
	viaAggregator?: boolean;
};

export async function runEntryAddEncrypted(
	ctx: EncryptContext,
	name: string,
...
export async function runEntryDecrypt(
	ctx: DecryptContext,
	entryId: string,
	revisionId: string | undefined,
	options: DecryptOptions,
): Promise<void> {
	// Validate the output mode before paying for the Walrus read and the Seal
	// key-server round-trip.
...
export function registerEncryptedEntryCommands(entry: Command): void {
	...
	collectionOption(
		...
		await runEntryAddEncrypted(
			await buildEncryptContext(command),
			name,
			options,
		);
	});
	...
	collectionOption(
		publicationOption(publisherCapOption(viaAggregatorOption(decrypt))),
	).action(
		async (
			entryId: string,
			revisionId: string | undefined,
			...
			await runEntryDecrypt(
				await buildDecryptContext(command, {
