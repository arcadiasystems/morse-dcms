...
function makeObjectFetcher(overrides?: any): ObjectFetcher {
	return {
		getObject: mock(async ({ objectId }) => ({
			object: makeObject(objectId, { name: "Test" }),
		})),
...
test("listPublications fetches publication by publication_id from cap json, not the cap objectId", async () => {
	const client = makeObjectFetcher({
		...
		getObject: mock(async () => ({
			object: makeObject("0xPUB", { name: "My Blog" }),
		})),
	});

	await listPublications(client, keypair, "0xADDR");

	const getObjectCalls = (client.getObject as ReturnType<typeof mock>).mock
		.calls;
...
test("listPublications queries with the correct OwnerCap type", async () => {
	...
	const [input] = (client.listOwnedObjects as ReturnType<typeof mock>).mock
		.calls[0]!;
...
test("deletePublication fetches the publication before executing the transaction", async () => {
	...
	const getObjectCalls = (client.getObject as ReturnType<typeof mock>).mock
		.calls;
	const signCalls = (
		client.signAndExecuteTransaction as ReturnType<typeof mock>
	).mock.calls;
...
test("addEntry resolves PublisherCap and executes transaction", async () => {
	...
	const client = makeCollectionManager(undefined, {
		waitForTransaction: mock(async () => effectsResult),
	});
	...
	expect(receipt.digest).toBe("abc123");
	expect(receipt.gasUsedMist).toBe(2_500_000n);
	const signCalls = (
		client.signAndExecuteTransaction as ReturnType<typeof mock>
	).mock.calls;
	expect(signCalls).toHaveLength(1);
...
test("createPublication calls correct Move target", async () => {
	...
	const calls = (executor.signAndExecuteTransaction as ReturnType<typeof mock>)
		.mock.calls;
...
test("formatMist formats whole SUI", () => {
	expect(formatMist(1_000_000_000n)).toBe("1 SUI");
});

test("formatMist formats fractional SUI", () => {
	expect(formatMist(1_500_000_000n)).toBe("1.5 SUI");
});

test("formatMist formats sub-unit", () => {
	expect(formatMist(500_000n)).toBe("0.0005 SUI");
});

test("formatMist uses custom token name", () => {
	expect(formatMist(2_000_000_000n, "WAL")).toBe("2 WAL");
});
